...
<main class={`h-screen w-screen flex flex-col py-16 px-4`}>
    ...
    <ul class={`flex flex-col gap-2 mt-4`}>
        {#each belongings as item (item.id)}
            <li class={`flex flex-col border-2 rounded-md px-4 py-2`}>
                <div class={`flex justify-between items-center`}>
                    <h3>{item.luggage ? 'Luggage' : 'Mattress'}</h3>


                    <div class={`flex gap-4 justify-center items-center`}>
                        <button onclick={() => qrCode = item.id} aria-label="Display QR">
                        ...
                        <div class={`h-[12px] aspect-square rounded-full ${item.isCheckedIn ? 'bg-green-400' : item.checkedOutAt ? 'bg-purple-500' : 'bg-amber-300'}`}></div>
                    </div>
                </div>
...
{#if isOpen}
    ...
    <div class={`z-50 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-8 bg-neutral-200/40 backdrop-blur-sm flex flex-col px-4 py-2 rounded-lg border-2 border-neutral-400`}>
        ...
        >
            ...
            <div class={`z-50 flex justify-center items-center gap-2`}>
                <button type="button" onclick={() => isLuggage = true} class={`z-50 w-1/2 border px-2 py-2 rounded-md ${isLuggage ? 'border-green-400' : ''}`}>Luggage</button>
                <button type="button" onclick={() => isLuggage = false} class={`z-50 w-1/2 border px-2 py-2 rounded-md ${!isLuggage ? 'border-green-400' : ''}`}>Mattress</button>
            </div>
